<template>
	<view>
		<!-- 按钮 -->
		<button v-if=!isOpenTypeBtn :class="['buttonBorder',!_rotate?'dlbutton':'dlbutton_loading']" :style="{'background':bgColor, 'color': fontColor}">
			<view :class="_rotate?'rotate_loop':''">
				<text v-if="_rotate" class="cuIcon cuIcon-loading1 "></text>
				<text v-if="!_rotate">{{ text }}</text>
			</view>
		</button>
		<button v-if=isOpenTypeBtn    :open-type="openType" @getuserinfo="getuserinfo" :class="['buttonBorder',!_rotate?'dlbutton':'dlbutton_loading']" :style="{'background':bgColor, 'color': fontColor}">
			<view :class="_rotate?'rotate_loop':''">
				<text v-if="_rotate" class="cuIcon cuIcon-loading1 "></text>
				<text v-if="!_rotate">{{ text }}</text>
			</view>
	...
	export default{
		props:{
			...
			bgColor:{
				//按钮背景颜色
				type: String,
				default: "linear-gradient(to right, rgba(9, 198, 192, 1.0), rgba(9, 198, 192, 1.0))",
			},
			fontColor:{
				//按钮字体颜色
				type: String,
